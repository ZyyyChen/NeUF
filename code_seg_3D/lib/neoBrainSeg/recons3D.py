...
def get_imgs_cc_seqdyn_2(data_recal, img_cc_ref,
    ...
    fig_v, btn_X, btn_Y, btn_Z, slider, btn_prev, btn_next, switch_mode = \
        gui_lay3d(data_recal, fig=fig, gs=gs_viewer)
...
def determine_transform_points_v2_(img_cc_ref, img_cc_seqdyn, main_dir, ref, mode_debug):
    ...
    if file_points.exists():
        ...
        ax1 = fig.add_subplot(gs[0]); ax2 = fig.add_subplot(gs[1])#ax2 = fig.add_subplot(gs[1], sharey=ax1)
...
def determine_transform_v2(coord_pts_img_ref, coord_pts_img_seqdyn, data_recal,
    ...
    if debug_mode:
        ...
        if ref_file_path.exists():
            # scipy.io usually accepts Path objects directly, but str() ensures compatibility
        ...
        # Optimized: Replaced the for-loop with a direct NumPy slice assignment
        # Assuming params_patients is a 2D numpy array and X_transform has at least 7 elements
        params_patients[idx, :7] = X_transform[:7]

        # Optimized: Reused the ref_file_path variable instead of recreating the path
        sio.savemat(str(ref_file_path), {'params_patients_echo_v2': params_patients})
...
def data_pre_treatment_recons3D_2_v2(delta_X_cc, delta_X_seqdyn, debug_mode, main_dir, 
    ...
    try:
        ...
        if debug_mode:
            ...
            # Affichage
            #plt.show(block=True)
            plt.close() # Décommente cette ligne si tu veux que la fenêtre se ferme seule
...
def data_pre_treatment_recons3D_v2(d, delta_X_cc, delta_X_seqdyn, debug_mode, idx, main_dir, num_patient, ref):
    ...
    if should_reconstruct:
        ...
        try:
            ...
            print(f"Data loaded for {ref}, shape: {data_recal.shape}, dtype: {data_recal.dtype}", "min_:" , np.min(data_recal), "max_:", np.max(data_recal))

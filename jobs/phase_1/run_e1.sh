#!/bin/bash -l
# Run Phase 1 E1 independently: ungated raw low/high DUAL_HASH control.
# Example: qsub jobs/phase_1/run_e1.sh

#PBS -N neuf_phase1_e1
#PBS -q gpu
#PBS -l walltime=23:59:00
#PBS -l nodes=1:ppn=16:gpus=1:gpu48
#PBS -l mem=128gb
#PBS -o /misc/raid/zchen/Code/NeUF/qsub/logs/neuf/phase1_e1_stdout.log
#PBS -e /misc/raid/zchen/Code/NeUF/qsub/logs/neuf/phase1_e1_stderr.log
...
JOB_LOG="${LOG_DIR}/E1_${JOB_ID}_seed${SEED}.log"
echo "${JOB_ID}" > "${LOG_DIR}/E1_job_id.txt"
...
cmd=(
  ...
  --field-head dual_single_head_matched
  --encoding DUAL_HASH
  ...
  --dual-hf-activate-ratio 0.2
  --dual-hf-max-weight 1.0
  --dual-no-gate
)

START_TIME="$(date --iso-8601=seconds)"
CONFIG_PATH="${RUN_ROOT}/${RUN_GROUP}_${RUN_CONTENT}_E1_config_${JOB_ID}.txt"
{
  ...
  echo "field_head=dual_single_head_matched"
  echo "encoding=DUAL_HASH"
  echo "seed=${SEED}"
  echo "nb_iters_max=${NB_ITERS_MAX}"
  echo "hf_activate_ratio=n/a"
...
echo "qsub stdout: ${REPO_DIR}/qsub/logs/neuf/phase1_e1_stdout.log"
echo "qsub stderr: ${REPO_DIR}/qsub/logs/neuf/phase1_e1_stderr.log"
printf "Command:"
printf " %q" "${cmd[@]}"
printf "\n"

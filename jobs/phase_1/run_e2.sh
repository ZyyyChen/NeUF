#!/bin/bash -l
# Run Phase 1 E2 independently: anatomy/speckle dual-branch DUAL_HASH.
# Example: qsub jobs/phase_1/run_e2.sh

#PBS -N neuf_phase1_e2
#PBS -q gpu
#PBS -l walltime=23:59:00
#PBS -l nodes=1:ppn=16:gpus=1:gpu48
#PBS -l mem=128gb
#PBS -o /misc/raid/zchen/Code/NeUF/qsub/logs/neuf/phase1_e2_stdout.log
#PBS -e /misc/raid/zchen/Code/NeUF/qsub/logs/neuf/phase1_e2_stderr.log
...
JOB_LOG="${LOG_DIR}/E2_${JOB_ID}_seed${SEED}.log"
echo "${JOB_ID}" > "${LOG_DIR}/E2_job_id.txt"
...
cmd=(
  ...
  --dual-base-resolution-low 16
  --dual-finest-resolution-low 64
  --dual-base-resolution-high 64
  --dual-finest-resolution-high 512
  --dual-no-gate
)

START_TIME="$(date --iso-8601=seconds)"
CONFIG_PATH="${RUN_ROOT}/${RUN_GROUP}_${RUN_CONTENT}_E2_config_${JOB_ID}.txt"
{
  ...
  echo "field_head=anatomy_speckle_v1"
  echo "encoding=DUAL_HASH"
  echo "seed=${SEED}"
  echo "nb_iters_max=${NB_ITERS_MAX}"
  echo "training=joint"
  echo "loss=masked_mse"
  echo "use_gate=false"
...
echo "E2 training: joint low/high, masked MSE, no gate"
echo "Dataset: ${DATASET_PATH}"
echo "Result directory: ${RUN_ROOT}"
echo "Experiment log: ${JOB_LOG}"
echo "qsub stdout: ${REPO_DIR}/qsub/logs/neuf/phase1_e2_stdout.log"
echo "qsub stderr: ${REPO_DIR}/qsub/logs/neuf/phase1_e2_stderr.log"
printf "Command:"
printf " %q" "${cmd[@]}"
printf "\n"

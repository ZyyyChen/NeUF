...
# python -m neuf.export_recons3d_style_baseline --ckpt latest/ckpt.pkl --output exports/recons3d_style
...
def load_dataset_from_ckpt(ckpt_path: Path):
    ...
    baked_dataset = dataset.Dataset.open_from_save(
        dataset_path,
        map_location=LOAD_DEVICE,
    )
    return baked_dataset, dataset_path, ckpt
...
def voxelize_observed_recal(
    ...
    for frame_idx in tqdm(range(num_frames), desc="Stage 1 observed voxelization"):
        ...
        inside_mask = sector_mask_hw.reshape(-1) & (
...
def voxelize_recons_intermediate(
    ...
    for sag_idx in tqdm(range(2, n_sag - 2), desc="Stage 2 recons interpolation"):
        ...
        for sample_idx in range(num_samples):
            ...
            valid_depth = depth_idx[valid_indices]
            valid_j[valid_indices] &= sector_mask_hw[valid_depth, sag_idx]
            valid_m1[valid_indices] &= sector_mask_hw[valid_depth, sag_idx - 1]
            valid_p1[valid_indices] &= sector_mask_hw[valid_depth, sag_idx + 1]

            safe_dist = np.where(dist < 1e-8, 1e-8, dist)
            w0 = np.zeros_like(dist, dtype=np.float64)
            w0[valid_j] = 1.0 / safe_dist[valid_j]

            accum_intensity[valid_j] += w0[valid_j] * plane_j[frame_idx[valid_j], depth_idx[valid_j]]
            accum_world[valid_j] += w0[valid_j, None] * world_j[frame_idx[valid_j], depth_idx[valid_j]]
            sum_w[valid_j] += w0[valid_j]

            w1 = np.zeros_like(dist, dtype=np.float64)
            w1[valid] = 1.0 / np.sqrt(safe_dist[valid] ** 2 + 1.0)
            accum_intensity[valid_m1] += w1[valid_m1] * plane_m1[
                frame_idx[valid_m1], depth_idx[valid_m1]
            ]
            accum_world[valid_m1] += w1[valid_m1, None] * world_m1[
                frame_idx[valid_m1], depth_idx[valid_m1]
            ]
            sum_w[valid_m1] += w1[valid_m1]
            accum_intensity[valid_p1] += w1[valid_p1] * plane_p1[
                frame_idx[valid_p1], depth_idx[valid_p1]
            ]
            accum_world[valid_p1] += w1[valid_p1, None] * world_p1[
                frame_idx[valid_p1], depth_idx[valid_p1]
            ]
            sum_w[valid_p1] += w1[valid_p1]
...
def main():
    ...
    if args.bounds_dataset is not None:
        bounds_ds = dataset.Dataset.open_from_save(
            args.bounds_dataset,
            map_location=LOAD_DEVICE,
        )
    ...
    bb_min = np.asarray(baked_dataset.point_min, dtype=np.float32)
    bb_max = np.asarray(baked_dataset.point_max, dtype=np.float32)
    ...
    print(f"Calibrated dataset bbox min/max: {bb_min.tolist()} / {bb_max.tolist()}")
    ...
    sector_mask_hw = (
        baked_dataset.get_sector_mask().detach().cpu().numpy().astype(bool, copy=False)
    )
    if sector_mask_hw.shape != (data_recal.shape[2], data_recal.shape[1]):
        raise ValueError(
            "Saved recons data shape does not match the mandatory ultrasound sector "
            f"mask: data_recal={data_recal.shape}, mask={sector_mask_hw.shape}"
        )
    ...
    metadata = {
        ...
        "dataset_physical_calibration": baked_dataset.physical_calibration_signature(),
        "ultrasound_sector_mask": baked_dataset.sector_mask_signature(),

...
# python -m neuf.recons3d_exact_from_saved \
...
def resolve_dataset_pkl(data_recal_mat: Path, dataset_pkl: Path | None) -> Path:
    ...
    candidates.extend(
        [
            recal_dir / "us_recal_original" / "baked_dataset_physical.pkl",
...
def load_baked_dataset(dataset_pkl: Path):
    return dataset.Dataset.open_from_save(
        dataset_pkl,
...
def data_interpolation_v2_25D_3(
    data_recal,
    nb_pts,
    multi_img,
    xp,
    ...
    _process_events()
    sector_mask_hw = np.asarray(sector_mask_hw, dtype=bool)
    if sector_mask_hw.shape != (data_recal.shape[2], data_recal.shape[1]):
        raise ValueError(
            "data_recal does not match the mandatory ultrasound sector mask: "
            f"data={data_recal.shape}, mask={sector_mask_hw.shape}"
        )
    data_3D_mask = np.zeros_like(data_int_mask_full, dtype=np.uint8)
    ...
    for j in tqdm.tqdm(active_sag_indices_zero_based, desc="Sagittal interpolation", dynamic_ncols=True):
        ...
        for k in range(I_complet_1.shape[1]):
            idx_k = I_complet_1[:, k]
            depth_idx = np.remainder(idx_k, n_height)
            w = 1 / d_complet_1[:, k]
            vals = data_recal_j.ravel()[idx_k]
            valid = sector_mask_hw[depth_idx, j]
            valid_w = w * valid
            accum += valid_w * vals
            sum_w += valid_w
            ...
            if nb_pts == 3:
                ...
                valid_m1 = w1 * sector_mask_hw[depth_idx, j - 1]
                valid_p1 = w1 * sector_mask_hw[depth_idx, j + 1]
                accum += valid_m1 * vals_m1 + valid_p1 * vals_p1
                sum_w += valid_m1 + valid_p1
            elif nb_pts == 5:
                w1 = 1 / d_complet_1_1[:, k]
                vals_m1 = data_recal_m1.ravel()[idx_k]
                vals_p1 = data_recal_p1.ravel()[idx_k]
                ...
                valid_m2 = w2 * sector_mask_hw[depth_idx, j - 2]
                valid_p2 = w2 * sector_mask_hw[depth_idx, j + 2]
                accum += valid_m2 * vals_m2 + valid_p2 * vals_p2
                sum_w += valid_m2 + valid_p2
        ...
        if multi_img == 1:
            for k in range(I_complet_2.shape[1]):
                ...
                w = 1 / d_complet_2[:, k]
                vals = data_recal_j.ravel()[idx_k]
                valid = w * sector_mask_hw[depth_idx, j]
                accum += valid * vals
                sum_w += valid
        ...
        has_valid_support = sum_w > 0
        accum[has_valid_support] /= sum_w[has_valid_support]

        data_int_temp = np.zeros((x_max, z_max))
        data_int_temp.ravel()[idx_mask] = accum

        data_int[:, j, :] = data_int_temp
        mask_temp = np.zeros((x_max, z_max), dtype=np.uint8)
        mask_temp.ravel()[idx_mask] = has_valid_support.astype(np.uint8)
        data_3D_mask[:, j, :] = mask_temp
        if j % 10 == 0:

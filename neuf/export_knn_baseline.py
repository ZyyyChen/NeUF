...
Usage
-----
python -m neuf.export_knn_baseline --ckpt latest/ckpt.pkl --output exports/knn_baseline
python -m neuf.export_knn_baseline --ckpt latest/ckpt.pkl --output exports/knn_baseline \\
...
def collect_observations(baked_dataset) -> tuple[np.ndarray, np.ndarray]:
    ...
    all_values = []
    sector_mask = (
        baked_dataset.get_sector_mask(flatten=True)
        .detach()
        .cpu()
        .numpy()
        .reshape(-1)
    ...
    def _add_slices(slices, get_pixels):
        for idx, sl in enumerate(slices):
            ...
            all_points.append(points_world[sector_mask].astype(np.float32))
            all_values.append(pixels_np[sector_mask])
...
def main():
    ...
    if args.use_bbox_mask:
        ...
        print(f"  calibrated dataset bb_min: {bb_min}")
        print(f"  calibrated dataset bb_max: {bb_max}")
